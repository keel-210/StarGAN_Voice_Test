...
def saveZ_datas(datas,path,attribute,phase):
	...
	for i in range(len(datas)):
		np.savez_compressed(path+str('{0:03d}'.format(i + j))+'.npz',FFT=datas[i],attr=attribute,phase = phase[i])
...
def main():
	...
	parser.add_argument('--out_path', type=str, default='./data_test/')
	...
	parser.add_argument('--attr',metavar='N', type=int, nargs='+',   default=[1,0,0,1,0,0,0])
