...
f0 = pysptk.sptk.swipe(data_raw, wav_bps, hopSize, max=5000, threshold=0.3, otype='f0').astype('float64')
pitch = pysptk.sptk.excite(pitch, hopsize=hopSize)
pitch = np.array([p if p > 2 else p/100 for p in pitch])
...
vocode = pw.synthesize(f0, sp, pitch, wav_bps)
